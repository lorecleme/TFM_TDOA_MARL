...
class MPEWrapper(MultiAgentEnv):
    def __init__(self, key, time_limit, **kwargs):
        ...
        scenario_args = {k: v for k, v in kwargs.items()
                         if k in ('num_agents', 'num_landmarks',
                                  'agent_max_speed', 'agent_accel',
                                  'target_movable', 'target_speed_mult', 'noise_std',
                                  'reward_dist_coeff', 'reward_surround_coeff',
                                  'reward_triangle_coeff', 'reward_triangle_continuous',
                                  'reward_triangle_temperature', 'reward_rmse_coeff',
                                  'reward_coverage_coeff', 'reward_collision_coeff',
                                  'surround_gate_distance', 'surround_gate_softness',
                                  'reward_simple_dist_coeff', 'reward_simple_dist_metric',
                          'soft_assign_temperature', 'reward_stillness_coeff',
                          'reward_balance_coeff', 'hide_target_positions',
                          'target_pos_mode')}
        world = scenario.make_world(**scenario_args)
        ...
        self.episode_surround    = np.zeros(self.episode_limit)
        self.episode_min_dist    = np.zeros(self.episode_limit)
    ...
    def step(self, actions, animate=False):
        ...
        if animate:
            ...
            world_info = info.get("world", {})
            self.episode_surround[self.t] = world_info.get("surround_quality", 0.0)
            self.episode_min_dist[self.t] = world_info.get("min_dist_to_target", 0.0)
        self.t += 1
    ...
    def reset(self):
        ...
        self.episode_surround.fill(0.)
        self.episode_min_dist.fill(0.)
    ...
    def save_animation(self, path):
        if self.t == 0:
            print(f"[anim] skipped — t=0, no frames recorded")
            return
        print(f"[anim] saving {self.t} frames to {path}")
        anim = MPEAnimator(self.agent_positions[:,:self.t,:],
                           self.landmark_positions[:,:self.t,:],
                           self.episode_rewards_all[:self.t],
                           episode_surround=self.episode_surround[:self.t],
                           episode_min_dist=self.episode_min_dist[:self.t])
