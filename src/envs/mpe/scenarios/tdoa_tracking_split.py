"""TDOA tracking with targets moving in opposite directions — split test."""
import numpy as np
from .tdoa_tracking import Scenario as BaseScenario, WORLD_MIN, WORLD_MAX, WORLD_SPAN, WORLD_DIAG

class SplitWorld(type(BaseScenario().make_world()).__class__):
    """World where two targets move in opposite directions."""
    ...
    def step(self):
        for i, landmark in enumerate(self.landmarks):
            if landmark.movable and getattr(landmark, 'is_target', False):
                self._update_target_opposite(landmark, i)
        super().step()

    def _update_target_opposite(self, target, idx):
        """Target 0 moves right+up, target 1 moves left+down."""
        speed = 8.0 * getattr(target, 'speed_mult', 1.0)
        if idx == 0:
            direction = np.array([0.7, 0.7]) / np.sqrt(2) * speed
        else:
            direction = np.array([-0.7, -0.7]) / np.sqrt(2) * speed

        new_pos = target.state.p_pos + direction
        ...
        for d in range(self.dim_p):
            if new_pos[d] > WORLD_MAX - margin:
                ...
                direction[d] = abs(direction[d]) * -1
            elif new_pos[d] < WORLD_MIN + margin:
                new_pos[d] = WORLD_MIN + margin
                direction[d] = abs(direction[d])

        target.state.p_vel = (new_pos - target.state.p_pos) / self.dt
        target.state.p_pos = new_pos
        self._wander_dir[idx] = direction


class Scenario(BaseScenario):
    """Same as TDOA tracking but targets move in opposite directions."""
    ...
    def make_world(self, *args, **kwargs):
        ...
        self._tdoa_pairs = list(BaseScenario.__dict__['_tdoa_pairs'].fget.__class__(None).__new__(None))
    ...
    def reset_world(self, world):
        """Targets start at opposite edges, agents in middle."""
        world._wander_dir.clear()
        world._wander_timer.clear()

        world.landmarks[0].state.p_pos = np.array([300.0, 300.0])
        world.landmarks[0].state.p_vel = np.zeros(2)
        world.landmarks[1].state.p_pos = np.array([700.0, 700.0])
        world.landmarks[1].state.p_vel = np.zeros(2)

        for agent in world.agents:
            agent.state.p_pos = np.array([500.0, 400.0]) + world.np_random.uniform(-100, 100, 2)
