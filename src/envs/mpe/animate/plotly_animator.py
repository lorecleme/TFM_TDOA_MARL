...
class MPEAnimator():
    ...
    def plot_static(self, frame):
        ...
        fig = make_subplots(rows=2, cols=2,
                            subplot_titles=("Episode", "Reward", "Surround quality", "Min distance to target"))
        ...
        for l in range(self.n_landmarks):  
            ...
            fig.add_trace(go.Scatter(x=self.landmark_positions[l, :frame, 0],
                                     ...
                                     marker_symbol='diamond',
                                     showlegend=False,
                                     marker=dict(size=5, color=px.colors.sequential.Greens[::-1][l], opacity=0.08)),
                         row=1, col=1)
        ...
        if self.episode_surround is not None:
            fig.add_trace(go.Scatter(x=np.arange(frame),
                                     y=self.episode_surround[:frame],
                                     mode='lines',
                                     marker=dict(color="blue"),
                                     name='Surround quality'),
                         row=2, col=1)

        # min distance
        if self.episode_min_dist is not None:
            fig.add_trace(go.Scatter(x=np.arange(frame),
                                     y=self.episode_min_dist[:frame],
                                     mode='lines',
