...
class EpisodeRunner:
    ...
    def __init__(self, args, logger=None):
        ...
        self.t_env = 0
        self.epsilon_offset = 0  # subtract from t_env for fine-tuning epsilon restart
    ...
    def run(self, test_mode=False, render=False, save_animation=False, benchmark_mode=False):
        ...
        while not terminated:
            ...
            actions = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env - self.epsilon_offset, test_mode=test_mode)
        ...
        # final-step-only metrics — env_info on the terminal step carries them
        for final_key in ("tdoa_rmse_m", "tdoa_converged"):
            if final_key in env_info:
                cur_stats["final_" + final_key] = cur_stats.get("final_" + final_key, 0) + env_info[final_key]
