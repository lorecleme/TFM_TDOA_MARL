...
class TransformerMixer(nn.Module):
    ...
    def forward(self, qvals, hidden_states, hyper_weights, states, obs):
        ...
        # dynamic entity count so models transfer across agent/target counts
        if self.custom_space:
            inputs = states.reshape(b, -1, self.feat_dim)
        else:
            inputs = obs.reshape(b, -1, self.feat_dim)
        ...
        n_agents = hidden_states.shape[1]  # dynamic to support transfer
        w1 = embs[:, -3-n_agents:-3, :]
